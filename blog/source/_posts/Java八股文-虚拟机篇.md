---
title: Java 八股文背诵版 - 虚拟机篇（29 道高频面试题）
...
- JDK8 之前，放在方法区，大小受限于方法区。
- JDK8 将运行时常量池存放堆中。

## 84. 简述直接内存

直接内存也称为堆外内存，就是把内存对象分配在 JVM 堆外的内存区域。这部分内存不是虚拟机管理，而是由操作系统来管理。Java 通过 DirectByteBuffer 对其进行操作，避免了在 Java 堆和 Native 堆来回复制数据。
...
## 86. 简述 JVM 给对象分配内存的策略

- **指针碰撞**：这种方式在内存中放一个指针作为分界指示器将使用过的内存放在一边，空闲放在另一边，通过指针挪动完成分配。
...
**3）对齐填充**：起占位作用。HotSpot 虚拟机要求对象的起始地址必须是 8 的整数倍，因此需要对齐填充。
...
## 94. 简述 Parallel Scavenge 垃圾收集器

注重吞吐量，即 CPU 运行代码时间 / CPU 耗时总时间（CPU 运行代码时间 + 垃圾回收时间）。新生代使用标记复制算法，老年代使用标记整理算法。

## 95. 简述 CMS 垃圾收集器

CMS 垃圾收集器注重最短时间停顿。CMS 垃圾收集器为最早提出的并发收集器，垃圾收集线程与用户线程同时工作。采用标记清除算法。

该收集器分为以下几个步骤：

1. **初始标记**：暂停其他线程（stop the world），标记与 GC roots 直接关联的对象。
2. **并发标记**：可达性分析过程（程序不会停顿）。
3. **并发预清理**：查找执行并发标记阶段从年轻代晋升到老年代的对象，重新标记，暂停虚拟机（stop the world）扫描 CMS 堆中剩余对象。
4. **并发清除**：清理垃圾对象（程序不会停顿）。
5. **并发重置**：重置 CMS 收集器的数据结构。
...
1. **初始标记**：标记与 GC roots 直接关联的对象。
2. **并发标记**：可达性分析。
3. **最终标记**：对并发标记过程中，用户线程修改的对象再次标记一下。
4. **筛选回收**：对各个 Region 的回收价值和成本进行排序，然后根据用户所期望的 GC 停顿时间制定回收计划并回收。
...
- **ExtensionClassLoader 扩展类加载器**：加载 `/lib/ext` 目录下的 jar 包和类。由 Java 编写。
